...
import {$Enums, PointAllocationType} from '.prisma/client';
...
const reCreateLabels = async (context: Context) => {
  ...
  await Promise.all(labelNames.map(
      labelName => context.octokit.issues.deleteLabel(
          {...context.repo(), name: labelName})));
  await Promise.all(labelsWithColors.map(
      label => context.octokit.issues.createLabel(
          {...context.repo(), ...label})));
};
...
const reCreateIssue = async (context: Context<'issues'>) => {
  ...
  if (issue) await prisma.pointAllocation.updateMany({
    where: {
      issueId: issue.id,
    },
    data: {
...
const comment = (context: Context, body: string) => {
  return context.octokit.issues.createComment(
      {...context.issue(), body: commentWithMarkdown(body)});
};

const checkIfCommenterIsAdmin = (context: Context<'issue_comment.created'>) => adminUsers.includes(
    context.payload.comment.user.login);

const checkAndHandlePointRevaluationNeededLabel = async (context: Context<'issue_comment.created'>) => {
  const hasLabel = context.payload.issue.labels?.find(label => label.name === "need-point-revaluation");
  let needLabel = false
  ...
  if (!needLabel && hasLabel) {
    await context.octokit.issues.removeLabel(
        {...context.issue(), name: 'need-point-revaluation'});
  }
  if (needLabel && !hasLabel) {
    await context.octokit.issues.addLabels(
        {...context.issue(), labels: ['need-point-revaluation']});
  }
}
...
export = (app: Probot) => {
  ...
  app.on('issues.opened', async (context) => {
    ...
    if (!labels?.includes('bug') && !labels?.includes('enhancement')) {
      await comment(context, commandErrorWithMarkdown(
      ...
      switch (parts[1]) {
        ...
        case 'unassign':
          await context.octokit.issues.removeAssignees(
              {...context.issue(), assignees: [parts[2]]});
          break;
        case 'label':
          await context.octokit.issues.addLabels(
              {...context.issue(), labels: [parts[2]]});
          break;
        case 'unlabel':
          await context.octokit.issues.removeLabel(
              {...context.issue(), name: parts[2]});
          break;
        case 'close':
        ...
        case 'comment':
          await context.octokit.issues.createComment(
              {...context.issue(), body: parts.slice(2).join(' ')});
          break;
        case 'point':
          if (!parts[2]) return;
          const issue = await prisma.issue.findUnique({
          ...
          switch (parts[2]) {
            ...
            case 'request':
              ...
              const allocatedTo = parts[3].startsWith('@') ? parts[3].slice(1) :  parts[3];
              const pointsRequested = parseInt(parts[4]);
              if (!pointsRequested || pointsRequested < 0 || isNaN(pointsRequested)) throw new Error(
                  'Invalid points');
              if (!Object.values(PointAllocationType).includes(parts[5] as any)) throw new Error(
                  'Invalid type. Use one of ' + Object.values(PointAllocationType).join(', '));
...
| --- | --- | --- | --- | --- | --- | --- | --- |
${pointAllocations.map(pa => `| ${pa.points} | ${pa.type} | ${pa.requestedBy} | ${pa.allocatedTo} | ${pa.approvedBy} | ${pa.approvedAt} | ${pa.rejectedBy} | ${pa.rejectedAt} |`).join('\n')}
`;
        ...
        case 'help':
          ...
          /bot point request <points> - Requests points to a user
          /bot point revoke <points> - Revokes point request
